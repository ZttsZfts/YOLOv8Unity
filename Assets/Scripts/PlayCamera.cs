...
public class PlayCamera : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        StartCoroutine(Illation());
    }


    private IEnumerator Illation()
    {
        while (true)
        {
            var cameraInput = new Mat(_webCamTexture.height, _webCamTexture.width, CvType.CV_8UC3);
            Utils.webCamTextureToMat(_webCamTexture, cameraInput);
            var cameraInputZoom = new Mat(640, 640, CvType.CV_8UC3);
            ...
            rawImage.texture = cameraInputTexture2D;
            var inputTensor = new Tensor(cameraInputTexture2D, 3);

            var outputTensor = _worker.Execute(inputTensor).PeekOutput();
